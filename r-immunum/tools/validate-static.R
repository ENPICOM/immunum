...
# Phase C.2: polars_* functions must be exported
phase_c2_exports <- c(
  "polars_number",
  "polars_segment",
  "polars_numbering_method",
  "polars_segmentation_method"
)
for (e in phase_c2_exports) {
...
# Phase D: validation pipeline functions must be exported
phase_d_exports <- c(
  "benchmark_threshold",
  "validation_fixtures"
)
for (e in phase_d_exports) {
  if (!any(grepl(sprintf("^export\\(%s\\)", e), ns_lines))) {
    fail(sprintf("NAMESPACE does not export %s", e))
  } else {
    cat("OK: NAMESPACE exports", e, "\n")
...
if (!any(grepl('immunum = \\{ path = "\\.\\./\\.\\./\\.\\.',  cargo_toml))) {
  fail("src/extendr/Cargo.toml does not use a path dep to the repo-root immunum crate")
} else {
  cat("OK: src/extendr/Cargo.toml uses path dep to repo-root immunum crate\n")
}

# 6. Phase B: lib.rs has the Annotator impl + module registration
...
# Phase C.2: lib.rs has the four polars batch primitives + their
# extendr_module! lines, and the polars helpers it depends on.
phase_c2_symbols <- c(
  "fn numbering_batch\\(",
...
# 7. Phase B: Rd files exist and check clean
...
# 8. Phase B: Annotator.Rd documents the four R6 methods (so R CMD check
...
# 9. Phase B: helper-fixtures.R + INIT_CASES present so the parametrized
...
# 10. Phase D: validation fixtures exist at ../fixtures/validation/ in
...
# 11. Phase D: BENCHMARKS.toml thresholds must match the baked-in
